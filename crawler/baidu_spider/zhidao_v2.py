...
def get_info(url):
    ...
    html = re.sub('<em>', '', html)
    html = re.sub('</em>', '', html)
    titles = re.findall('target="_blank" class="ti">([\s\S]*?)</a>\s+</dt>',
                        html)
...
'''
    ...
    html1 = get_html(url1)
    id1 = re.findall(u'<span class="f-lighter lh-22">共(.*?)条结果</span>',
                     html1)[0]
    id1 = re.sub(',', '', id1)
    id2 = int(id1) / 10
    ...
    if id2 >= 75:
        for id in range(0, 77):  #共76页
            ...
            url = "http://zhidao.baidu.com/search?pn=%s&word=%s" % (num,
                                                                    keyword)
            # print '正在挖掘',url,'第',id+1,'页的关键词'
            print '正在挖掘第', id + 1, '页的关键词'
            get_info(url)
        ...
        for id in range(0, id2):  #共76页
            ...
            url = "https://zhidao.baidu.com/search?pn=%s&word=%s" % (num,
                                                                     keyword)
            # print '正在挖掘',url,'第',id+1,'页的关键词'
            print '正在挖掘第', id + 1, '页的关键词'
            get_info(url)
